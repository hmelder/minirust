    ...
    implements MiniRustVisitor<MiniRustType>
{
    ...
    private borrowVariable(
        name: string,
        borrowType: 'mutable' | 'immutable',
        ...
        if (borrowType === 'mutable') {
            if (ownershipInfo.isBorrowed) {
                console.log(
                    `Conflict: '${name}' is already borrowed ${ownershipInfo.borrowType}`
                )
                this.addError(
    ...
    visitProg(ctx: ProgContext): MiniRustType {
        ...
        for (const child of ctx.children) {
            if (child instanceof ErrorNode) {
                this.addError(
                    `All statements and expressions must be inside functions.`,
                    child
                )
                return PrimitiveType.Error
            }
    ...
    visitLet_statement(ctx: Let_statementContext): MiniRustType {
        ...
        if (declaredType && initializerType) {
            if (
                ...
                !(initializerType === PrimitiveType.Bool)
            ) {
    ...
    visitBorrowExpr(
        ctx: ImmutableBorrowExprContext | MutableBorrowExprContext
    ): MiniRustType {
        const borrowType =
            ctx instanceof MutableBorrowExprContext ? 'mutable' : 'immutable'
        const exprCtx = ctx.expression()
    ...
    visitCallExpr(ctx: CallExprContext): MiniRustType {
        ...
        if (!fnType || fnType.kind !== 'function') {
            this.addError(
                `Call to undefined or non-function '${functionName}'`,
                ctx
            )
            return PrimitiveType.Error
        }
        ...
        for (let i = 0; i < args.length; i++) {
            ...
            if (!typesEqual(argType, paramTypes[i])) {
                this.addError(
                    `Argument ${i + 1} of '${functionName}' expected type '${
                        paramTypes[i]
                    }', got '${argType}'`,
                    args[i]
                )
